...
<head>
  ...
  <title>Fever Decision Support (MVP)</title>
  <style>
    :root { font-family: ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Arial; }
    body { margin: 0; background:#f8fafc; color:#0f172a; }
    .wrap { max-width: 980px; margin: 0 auto; padding: 24px 16px 48px; }
    .card { background:#fff; border:1px solid #e2e8f0; border-radius:16px; padding:16px; box-shadow:0 1px 1px rgba(15,23,42,.04); }
    h1 { margin:0 0 6px; font-size:22px; }
    .muted { color:#475569; font-size:13px; line-height:1.4; }
    .row { display:grid; grid-template-columns: 1fr; gap:14px; }
    @media(min-width: 880px){ .row { grid-template-columns: 1fr 1fr; } }
    label { display:block; font-size:13px; font-weight:600; color:#334155; margin-bottom:6px; }
    input, select { width:100%; padding:10px 12px; border:1px solid #cbd5e1; border-radius:12px; font-size:14px; background:#fff; }
    .btns { display:flex; gap:10px; margin-top:10px; }
    button { padding:10px 14px; border-radius:12px; border:1px solid #cbd5e1; background:#fff; font-weight:700; cursor:pointer; }
    button.primary { background:#0f172a; color:#fff; border-color:#0f172a; }
    button.primary:hover { background:#111c35; }
    button:hover { background:#f1f5f9; }
    .checks { display:grid; gap:8px; margin-top:6px; }
    .check { display:flex; gap:10px; align-items:flex-start; }
    .check input { width:auto; margin-top:3px; }
    .pill { display:inline-flex; align-items:center; gap:8px; border-radius:999px; padding:6px 10px; font-weight:800; font-size:12px; border:1px solid; }
    .pill.red { background:#fee2e2; border-color:#fecaca; color:#991b1b; }
    .pill.amber { background:#fef3c7; border-color:#fde68a; color:#92400e; }
    .pill.green { background:#d1fae5; border-color:#a7f3d0; color:#065f46; }
    .box { background:#f8fafc; border:1px solid #e2e8f0; border-radius:14px; padding:12px; }
    ul { margin: 8px 0 0 18px; }
    .error { background:#fef2f2; border:1px solid #fecaca; color:#991b1b; padding:10px 12px; border-radius:12px; font-size:13px; }
...
<body>
  <div class="wrap">
    <div class="topbar">
      <div>
        <h1 id="title">Ateş Yönetim Karar Destek (MVP)</h1>
        <div class="muted" id="subtitle">Bu araç bilgilendirme amaçlıdır; muayene ve hekim değerlendirmesi yerine geçmez.</div>
      </div>
      <div class="card" style="padding:12px 12px;">
        <label id="langLabel" style="margin-bottom:6px;">Dil</label>
        <select id="lang">
    ...
    <div style="margin-top:14px" class="card">
      <div class="check">
        ...
        <div class="muted" id="disclaimerCheck">Okudum, anladım. Bu aracın genel bilgilendirme amaçlı olduğunu kabul ediyorum.</div>
      </div>
    </div>
    ...
    <div class="row" style="margin-top:16px;">
      <!-- Inputs -->
      <div class="card">
        <h2 id="inputsTitle" style="margin:0 0 10px;font-size:16px;">Girdiler</h2>
        ...
        <div style="display:grid; gap:12px;">
          ...
          <div>
            ...
            <input id="weight" type="number" step="0.1" value="10" inputmode="decimal">
          </div>

          <div>
            <label id="tLabel">Ateş (°C)</label>
            <input id="temp" type="number" step="0.1" value="38.5" inputmode="decimal">
          </div>
          ...
          <div>
            <div style="font-size:13px;font-weight:700;color:#334155;" id="symptomsTitle">Eşlik eden bulgular</div>
            <div class="checks" id="symptomList"></div>
          </div>
      ...
      <!-- Output -->
      <div class="card">
        <h2 id="outTitle" style="margin:0 0 10px;font-size:16px;">Sonuç</h2>
        <div class="muted" id="hint">Girdileri doldurup “Hesapla”ya basın.</div>

        <div id="result" style="display:none; margin-top:10px;">
          <div id="triagePill" class="pill green">YEŞİL — Evde izlem uygun olabilir</div>
          ...
          <div>
            <div style="font-weight:900;" id="doseTitle">Doz hesaplayıcı (genel)</div>
            <div class="small" id="doseNote">Aşağıdaki hesaplama bilgilendirme amaçlıdır. Kullandığınız ilacın prospektüsünü ve hekim önerisini esas alın.</div>

            <div style="display:grid; gap:12px; margin-top:10px;">
              <!-- Paracetamol -->
              <div class="box">
                <div style="display:flex; justify-content:space-between; gap:10px; flex-wrap:wrap;">
                  <div style="font-weight:900;" id="paraTitle">Parasetamol</div>
                  <div class="small" id="paraInterval">Aralık: 4–6 saatte bir (günlük maksimumu aşmayın)</div>
                </div>
              ...
              <div class="box">
                <div style="display:flex; justify-content:space-between; gap:10px; flex-wrap:wrap;">
                  <div style="font-weight:900;" id="ibuTitle">İbuprofen</div>
                  <div class="small" id="ibuInterval">Aralık: 6–8 saatte bir (dehidratasyonda/şüphede kaçının)</div>
                </div>
              ...
              <div class="small" id="noData">Bu MVP veri saklamaz. Kullanıcı girdileri tarayıcıda işlenir.</div>
            </div>
          </div>
        </div>

        <div style="margin-top:14px" class="small" id="footerNote">
...
<script>
  const COPY = {
    tr: {
      title: "Ateş Yönetim Karar Destek (MVP)",
    ...
    en: {
      title: "Fever Decision Support (MVP)",
  ...
  function computeTriage({ageBand, tempC, duration, symptoms}){
    ...
    // age under 3 months conservative rules
    if (ageBand === "0_3m"){
      if (tempC >= 38.0) return "red";
      if (tempC >= 37.5) return "amber";
    }

    // high fever threshold conservative
  ...
  function renderSymptoms(){
    ...
    symptomKeys.forEach(key => {
      ...
      row.className = "check";
      row.innerHTML = `<input type="checkbox" id="sym_${key}"><div class="muted">${labels[key]}</div>`;
  ...
  function setText(){
    ...
    // update select option text for age/duration
  ...
  function showResult(level){
    ...
    pill.classList.remove("red","amber","green");
    pill.classList.add(level);
    pill.textContent = t.triage[level];

    const ul = $("advice");
    ul.innerHTML = "";
    t.advice[level].forEach(line => {
      const li = document.createElement("li");
      li.textContent = line;
      ul.appendChild(li);
    });

    // ibuprofen warning under 6 months: we only have age bands. show warning for 0-3m.
    const ageBand = $("ageBand").value;
    const warn = $("ibuWarn");
    if (ageBand === "0_3m") {
      warn.style.display = "block";
      warn.textContent = t.ibuWarn;
    } else {
  ...
  $("paraSusp").addEventListener("change", updateDoses);
  $("ibuSusp").addEventListener("change", updateDoses);
